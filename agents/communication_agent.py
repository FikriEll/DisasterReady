...
Generate laporan situasi otomatis dan personalisasi pesan menggunakan Claude API.

Model: Anthropic Claude (claude-sonnet-4-20250514)
Penyedia: Anthropic, PBC | https://www.anthropic.com/api
Penggunaan:
...
Keamanan data:
  - Tidak ada data pribadi warga yang dikirim ke Claude
...
class CommunicationAgent:
    """
    Agent komunikasi: Generate laporan dan pesan menggunakan Anthropic Claude API.
    ...
    def __init__(self, firebase=None, simulation_mode: bool = None):
        ...
        if ANTHROPIC_API_KEY:
            try:
                ...
                logger.info(f"✅ CommunicationAgent: Anthropic Claude ({CLAUDE_MODEL}) terhubung")
            except ImportError:
                logger.warning("Anthropic library tidak terinstall. Menggunakan template fallback.")
        else:
            logger.warning("⚠️  ANTHROPIC_API_KEY tidak ditemukan. Menggunakan template fallback.")
    ...
    async def generate_situation_report(
        ...
        if self.firebase:
            report_id = self.firebase.save_report(disaster_id, {
                ...
                "generated_by": "claude" if self._client else "template",
            })
...
Tulis langsung pesannya (tidak perlu pengantar)."""
        ...
        try:
            ...
            logger.error(f"Claude API error: {e}. Fallback ke template.")
            from agents.early_warning_agent import build_notification_message
            return build_notification_message(
                resident, vulnerability_score, alert_level,
                district_name, rainfall_mm, disaster_type
    ...
    async def _generate_with_claude(self, context: dict, report_type: str) -> str:
        """Generate report menggunakan Claude API."""
        import asyncio
