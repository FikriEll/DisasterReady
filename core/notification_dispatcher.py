...
Mengirim notifikasi early warning ke warga via WhatsApp Business (Twilio) / SMS fallback.

Channels:
1. WhatsApp Business via Twilio (primary)
2. Twilio SMS (fallback untuk warga tanpa WhatsApp)
...
class NotificationDispatcher:
    ...
    def __init__(self, simulation_mode: bool = None):
        ...
        if not self.simulation_mode:
            ...
            # Mode simulasi: coba init Telegram saja (bisa kirim notif nyata meski simulasi)
            tg_token = os.getenv("TELEGRAM_BOT_TOKEN")
            if tg_token:
                try:
                    import telegram
                    self._telegram_bot = telegram.Bot(token=tg_token)
                    logger.info("✅ Telegram Bot aktif (simulasi + notif nyata)")
    ...
    def _init_channels(self):
        ...
        if sid and token:
            try:
                ...
                logger.info("✅ Twilio WhatsApp & SMS terhubung")
            except Exception as e:
    ...
    async def _send_telegram(self, resident: dict, message: str, preview: str) -> NotificationResult:
        ...
        try:
            ...
            logger.info(f"✅ Telegram terkirim ke {resident.get('name')} (chat_id: {resident['telegram_id']})")
            return NotificationResult(resident["id"], NotificationChannel.TELEGRAM,
                                      NotificationStatus.SENT, preview)
        except Exception as e:
            logger.error(f"Telegram send error: {e}")
            return NotificationResult(resident["id"], NotificationChannel.TELEGRAM,
                                      NotificationStatus.FAILED, preview, error=str(e))
    ...
    async def send_demo_telegram(self, chat_id: str, message: str) -> dict:
        ...
        if not self._telegram_bot:
            return {"status": "error", "detail": "Telegram Bot belum dikonfigurasi. Set TELEGRAM_BOT_TOKEN di .env"}
        try:
            await self._telegram_bot.send_message(
                chat_id=chat_id,
                text=message,
